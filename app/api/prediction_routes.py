"""Smart Guardian - AI Prediction API routes (Dual-Mode: Sensor + EEG)."""
...
from app.schemas.prediction import (
    ...
    BatchPredictionRequest, BatchPredictionResponse, EEGSegmentRequest,
)
...
async def analyze_sensor_data(
    ...
    """Analyze sensor data for seizure detection (supports EEG + sensors)."""
    service = SeizureDetectionService(db)
    return await service.analyze(data)


@router.post("/analyze-eeg", response_model=PredictionResponse)
async def analyze_eeg_segment(
    data: EEGSegmentRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Analyze raw EEG time series segment for seizure detection."""
    service = SeizureDetectionService(db)
    return await service.analyze_eeg(data)
